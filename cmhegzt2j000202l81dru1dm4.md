...
When you push your applications beyond predictable user flows, monkey testing comes to the rescue. It helps in finding crashes, unhandled exceptions, memory leaks, and stability problems that structured test cases fail to uncover. If done properly, monkey testing provides a safety net - showing weaknesses of the software before any real users see them.
...
* **Regression Testing:** After a decent refactoring effort, but beforehand, if you have any [**unpredictable regressions**](https://keploy.io/blog/community/regression-testing-an-introductory-guide).
...
* **Ad-hoc Testing:** [Ad-hoc testing](https://keploy.io/blog/community/what-is-ad-hoc-testing) is usually a human-prompted test technique. A skilled, knowledgeable tester will randomly explore the application with their knowledge of the domain. The tester relies on instinct to identify faults when using ad-hoc testing.
